...
var Create = {
  contract: function(directory, name, callback) {
    ...
    var specifiedDirs = name.split("/");
    specifiedDirs.pop();
    
    for (let i = 0, slug = specifiedDirs[0]; i < specifiedDirs.length; i++) {
      let dir = path.join(directory, slug);
      if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir);
      }
      
      slug = path.join(slug, specifiedDirs[i + 1]);
    }

    copy.file(from, to, function(err) {
      if (err) return callback(err);
    
      var pureName = name.split('/').pop();
      replaceContents(to, templates.contract.name, pureName, callback);
    });
  },
